...
def test(args, T, dqn, val_mem, evaluate=False):
    ...
    if not evaluate:
        ...
        # Plot
        _plot_line(Ts, rewards, dqn.experiment + '_Rewards', path=dqn.plots_path)
        _plot_line(Ts, Qs, dqn.experiment + '_Q', path=dqn.plots_path)

        # Save model parameters if improved
        if avg_reward > best_avg_reward:
            best_avg_reward = avg_reward
            dqn.save(dqn.saved_model_path)

        # save the data that is being plotted - this might not work, not sure
        with open(os.path.join(dqn.data_save_path, dqn.experiment + '_data.json'), 'w') as f:
            json.dump({'Ts': Ts, 'Rewards': rewards, 'Qs': Qs}, f)
